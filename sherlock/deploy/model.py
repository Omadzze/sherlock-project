...
class CodeForCheckingThresholdScore:

    def __init__(self, class_names: list = None, unk_threshold: float = 0.5):
        self.lamb = 0.0001
        self.do = 0.35
        self.lr = 0.0001
        ...
        # Uncertainty threshold and class mapping
    ...
    def predict(self, X: pd.DataFrame, model_id: str = "sherlock") -> np.array:
        ...
        if self.class_names is None:
            raise ValueError(
                "class_names not set. Initialize with class_names list matching trained model."
            )

        # y_pred = self.predict_proba(X, model_id)
        proba = self.predict_proba(X, model_id)

        # added code
        top_idx = np.argmax(proba, axis=1)
        top_p = proba[np.arange(len(top_idx)), top_idx]

        # Map to class names and apply threshold
        labels = np.array(self.class_names, dtype=object)[top_idx].copy()
        labels[top_p < self.unk_threshold] = "unknown"
        return labels
        # top_idx = np.argmax(proba, axis = 1)
        # top_p = proba[np.arange(len(top_idx)), top_idx]
    ...
    def predict_with_confidences(
        ...
        """
        For each sample in X, return a list of up to top_k (label, probability) pairs,
        sorted by descending confidence.
        """
        if self.class_names is None:
            raise ValueError(
                "class_names not set. Initialize with the list of labels matching the trained model."
            )
        proba = self.predict_proba(X, model_id)
        ...
        for i in range(proba.shape[0]):
            top = []
            for j in sorted_idx[i, :top_k]:
                top.append((self.class_names[j], float(proba[i, j])))
            results.append(top)
        return results
